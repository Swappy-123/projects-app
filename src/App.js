...
class App extends Component {
  ...
  uniqueTab = value => {
    this.setState({appTabId: value})
  }

  getFilterList = () => {
    const {appTabId} = this.state
    const filterList = projectsList.filter(each => each.category === appTabId)
    return filterList
  }
        ...
        <ul className="tabs-container">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              uniqueTab={this.uniqueTab}
              isActive={appTabId === tabDetails.tabId}
            />
          ))}
        </ul>
